    ...
    def __init__(self, config, mode='train'):
        ...
        with open(csv_file, encoding='UTF-8-sig') as f2:
            ...
            for item in csv_reader:
                audio_path = os.path.join(self.data_root, 'CREMA-D-master/AudioWAV', item[1] + '.wav')
                visual_path = os.path.join(self.data_root, 'CREMA-D-master/VideoFlash', item[1])

                if os.path.exists(audio_path) and os.path.exists(visual_path):
                    self.image.append(visual_path)
                    self.audio.append(audio_path)
                    self.label.append(class_dict[item[1].split("_")[2]])
                    # n+=1
                    # if n==100:
                    #     break
                else:
